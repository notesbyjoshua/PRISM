...
def load_data(args):
    ...
    if healthy_path is None and disease_path is None:
        if bool(HEALTHY_FILE) != bool(DISEASE_FILE):
            raise ValueError("Set both HEALTHY_FILE and DISEASE_FILE in the Load data section.")
...
def bootstrap_effect_ci(x, y, iterations, rng):
    ...
    d_ci = np.quantile(delta_valid, [0.025, 0.975]) if len(delta_valid) else [np.nan, np.nan]
...
def add_phenotype_difference_score(results, q_column):
    ...
    stability = pd.to_numeric(results["effect_direction_bootstrap_stability"], errors="coerce").clip(lower=0, upper=1)
    sample_reliability = (results[["n_disease", "n_healthy"]].min(axis=1) / 50).clip(lower=0, upper=1)
    ...
    results["phenotype_difference_score_pdf"] = 100 * (
        0.45 * effect + 0.20 * confidence + 0.25 * stability + 0.10 * sample_reliability
    )
    return results
...
def robust_two_group_stats(disease_values, healthy_values, threshold):
    ...
    u_stat, p_value = mannwhitneyu(disease_clean, healthy_clean, alternative="two-sided")
    directional_auc, discrimination_auc = auc_from_u(u_stat, len(disease_clean), len(healthy_clean))
    return {
        "robust_n_disease": len(disease_clean),
        "robust_n_healthy": len(healthy_clean),
...
reliability_results["eligible"] = reliability_results["passes_missingness"] & reliability_results["passes_min_n"]
...
for feature in feature_cols:
    ...
    # Mann-Whitney U test
    try:
        u_stat, mw_p = mannwhitneyu(disease_values, healthy_values, alternative="two-sided")
    except ValueError:
        u_stat, mw_p = np.nan, np.nan

    # Welch t-test, included as a secondary parametric check
    try:
        t_stat, t_p = ttest_ind(disease_values, healthy_values, equal_var=False, nan_policy="omit")
    except ValueError:
...
binary_results["robust_mannwhitney_q_fdr"] = add_fdr(binary_results["robust_mannwhitney_p"])
binary_results["high_priority"] = (binary_results["mannwhitney_q_fdr"] < 0.05) & (
...
binary_stability = rank_stability(
    disease_df, healthy_df, binary_features, args.rank_bootstrap_iterations, args.top_k, rng
)
...
binary_results = binary_results.sort_values(["mannwhitney_q_fdr", "hedges_g"], ascending=[True, False])
...
for disease in disease_labels:
    ...
    for feature in feature_cols:
        ...
        try:
            ...
            u_stat, mw_p = np.nan, np.nan
        directional_auc, discrimination_auc = auc_from_u(u_stat, len(disease_values), len(healthy_values))
        g_low, g_high, delta_low, delta_high, direction_stability = bootstrap_effect_ci(
            disease_values, healthy_values, args.bootstrap_iterations, rng
        )
        result = {
            ...
            "roc_auc_directional": directional_auc,
            "roc_auc": discrimination_auc,
            "rank_biserial": rank_biserial_from_u(u_stat, len(disease_values), len(healthy_values)),
            "cliffs_delta": cliffs_delta(disease_values, healthy_values),
            "cliffs_delta_ci_95_low": delta_low,
            "cliffs_delta_ci_95_high": delta_high,
            "cohens_d": cohens_d(disease_values, healthy_values),
...
pairwise_results["mannwhitney_q_fdr_global"] = add_fdr(pairwise_results["mannwhitney_p"])
pairwise_results["robust_mannwhitney_q_fdr_global"] = add_fdr(pairwise_results["robust_mannwhitney_p"])
...
pairwise_results["high_priority"] = (pairwise_results["mannwhitney_q_fdr_global"] < 0.05) & (
...
for disease in disease_labels:
    ...
    stability = rank_stability(
        df[df["disease"] == disease], healthy_df, features, args.rank_bootstrap_iterations, args.top_k, rng
    )
    pairwise_results.loc[mask, "top_k_bootstrap_frequency"] = pairwise_results.loc[mask, "feature"].map(stability)
...
for feature, feature_results in pairwise_results.groupby("feature", sort=True):
    ...
    specificity = 1 - diseases_with_effect / diseases_tested if diseases_tested else np.nan
    median_effect = qualifying["hedges_g"].abs().median() if not qualifying.empty else 0.0
    specificity_rows.append(
        {
            ...
            "fraction_diseases_with_effect": (diseases_with_effect / diseases_tested if diseases_tested else np.nan),
            "feature_specificity_score": specificity,
            "median_abs_hedges_g_among_significant": median_effect,
            "max_abs_hedges_g_among_significant": (qualifying["hedges_g"].abs().max() if not qualifying.empty else 0.0),
...
feature_specificity_results = pd.DataFrame(specificity_rows).sort_values(
    ["specificity_weighted_effect_score", "n_diseases_with_significant_moderate_or_large_effect"],
...
pairwise_results = pairwise_results.sort_values(
    ["disease", "mannwhitney_q_fdr_within_disease", "hedges_g"], ascending=[True, True, False]
)
...
kw_results = []
all_labels = sorted(label for label, count in df["disease"].value_counts().items() if count >= args.min_group_n)
for feature in feature_cols:
    groups = []
    ...
    for label in all_labels:
        ...
        if len(values) < args.min_group_n or label_df[feature].isna().mean() > args.max_missing_fraction:
    ...
    kw_results.append(
        {
            ...
            "robust_epsilon_squared": epsilon_squared_kruskal(robust_h, robust_n, k_groups),
        }
    )
...
kw_results["high_priority"] = (kw_results["kruskal_q_fdr"] < 0.05) & (
    kw_results["epsilon_squared"] >= args.kw_effect_threshold
)
kw_results = kw_results.sort_values(["kruskal_q_fdr", "epsilon_squared"], ascending=[True, False])
...
binary_results = add_phenotype_difference_score(binary_results, "mannwhitney_q_fdr")
pairwise_results = add_phenotype_difference_score(pairwise_results, "mannwhitney_q_fdr_global")
...
print(
    binary_results.sort_values("phenotype_difference_score_pdf", ascending=False)[
        ["feature", "phenotype_difference_score_pdf"]
    ].head(20)
)
print("\nTop Phenotype Difference Scores (each disease vs healthy):")
print(
    pairwise_results.sort_values("phenotype_difference_score_pdf", ascending=False)[
        ["disease", "feature", "phenotype_difference_score_pdf"]
    ].head(20)
)
...
SIGNIFICANCE_Q = 0.05
top_binary = binary_results[binary_results["mannwhitney_q_fdr"] < SIGNIFICANCE_Q].copy()
top_pairwise = pairwise_results[pairwise_results["mannwhitney_q_fdr_global"] < SIGNIFICANCE_Q].copy()
top_kw = kw_results[kw_results["kruskal_q_fdr"] < SIGNIFICANCE_Q].copy()
...
high_priority_binary.to_csv(OUTPUT_DIR / "high_priority_healthy_vs_disease.csv", index=False)
high_priority_pairwise.to_csv(OUTPUT_DIR / "high_priority_each_disease_vs_healthy.csv", index=False)
high_priority_kw.to_csv(OUTPUT_DIR / "high_priority_kruskal_all_groups.csv", index=False)
...
feature_specificity_results.to_csv(OUTPUT_DIR / "feature_specificity.csv", index=False)
