...
extractor = GeometricFeatureExtractor(yaw_thresh=15, pitch_thresh=15, roll_thresh=10)
...
labels = labels[labels["age"] == TARGET_AGE].copy()
print()
print(f"FairFace images with age {TARGET_AGE}: " f"{len(labels):,}")
if len(labels) == 0:
    raise ValueError(f"No images found with age label {TARGET_AGE!r}.")
...
race_counts = labels["race"].value_counts()
races = sorted(labels["race"].dropna().unique())
print()
print(f"Available race counts for age {TARGET_AGE}:")
for race in races:
    print(f"  {race}: " f"{race_counts.get(race, 0):,}")

# ============================================================
# CALCULATE BALANCED QUOTAS
# ============================================================

base = TOTAL_IMAGES // len(races)
remainder = TOTAL_IMAGES % len(races)
quotas = {race: base for race in races}

# Give the extra +1 slots to groups
# with the most available candidates
extra_races = race_counts.sort_values(ascending=False).index[:remainder]
for race in extra_races:
    quotas[race] += 1
print()
print("Target distribution:")
for race in races:
    print(f"  {race}: " f"{quotas[race]}")
print(f"\nTOTAL: " f"{sum(quotas.values())}")
...
print()
print("Checking whether each race " "has enough age 3-9 candidates...")
for race in races:
    available = race_counts.get(race, 0)
    target = quotas[race]
    if available < target:
        print(
...
with zipfile.ZipFile(ZIP_PATH, "r") as zf:
    ...
    print(f"Indexed " f"{len(zip_lookup):,} " f"ZIP files.")
    ...
    for race in races:
        target = quotas[race]
        candidates = labels[labels["race"] == race].copy()

        # Randomize candidate order
        order = rng.permutation(len(candidates))
        candidates = candidates.iloc[order]
        accepted = 0
        attempted = 0
        non_frontal = 0
        no_face = 0
        ...
        for _, row in candidates.iterrows():
            ...
            attempted += 1
            zip_key = row["zip_key"]
            member = zip_lookup.get(zip_key)
            if member is None:
                missing_zip += 1
                continue
            suffix = Path(zip_key).suffix
            try:
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    temp_path = Path(tmpdir) / f"face{suffix}"
                    ...
                    feats = extractor.extract(str(temp_path), frontal_check=True)
                ...
                if feats.get("frontal_ok") is not True:
                    non_frontal += 1
                    continue
                ...
                record = row.to_dict()
                record["source"] = "FairFace"
                record["diagnosis"] = "Healthy"
                path = PurePosixPath(zip_key)
                record["split"] = path.parent.name
                ...
                errors += 1
                print(f"\nError with " f"{zip_key}: {e}")
        print()
        print()
        print(f"{race} finished:")
        print(f"  accepted:    {accepted}")
        print(f"  attempted:   {attempted}")
        print(f"  non-frontal: {non_frontal}")
...
print()
print("Frontal status:")
print(df["frontal_ok"].value_counts(dropna=False))
